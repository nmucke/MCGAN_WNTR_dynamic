...
class TrainAdvAE():
    def __init__(self, generator, critic, encoder,
        ...
        self.par_dim = self.generator.par_dim
    ...
    def train_epoch(self, dataloader):
        ...
        for bidx, (real_data, pars) in pbar:

            real_data = real_data.view(-1, real_data.size(-2), real_data.size(-1))
            pars = pars.view(-1, self.par_dim)
            ...
            # reconstruction loss
            recon_loss = self.recon_train_step(real_data, pars)
    ...
    def recon_train_step(self, real_data, pars):

        self.encoder_recon_optimizer.zero_grad()
        self.generator_optimizer.zero_grad()

        z_fake = self.encoder(real_data)
        fake_data = self.generator(z_fake, pars)
